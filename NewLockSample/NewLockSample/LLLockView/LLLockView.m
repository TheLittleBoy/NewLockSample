...
#define kLLLineColor [UIColor colorWithRed:0.11 green:0.64 blue:0.89 alpha:1] // 线条色蓝
#define kLLLineColorWrong [UIColor colorWithRed:0.96 green:0.33 blue:0.31 alpha:1] // 线条色红
#define KImageNone  @"lock_btn_none"  //圆圈图片
#define KImageSel   @"lock_btn_sel"   //蓝色圆圈图片
#define KImageError @"lock_btn_error" //红色圆圈图片
...
//        LLLog(@"每个圆点位置 %d,%d", x, y);
        ...
        [button setBackgroundImage:[UIImage imageNamed:KImageNone] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:KImageSel] forState:UIControlStateSelected];
...
- (void)showErrorCircles:(NSString*)string
{
    ...
    for (UIButton* button in buttonArray) {
        if (button.selected) {
            [button setBackgroundImage:[UIImage imageNamed:KImageError] forState:UIControlStateSelected];
        }
